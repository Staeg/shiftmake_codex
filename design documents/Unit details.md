...
- `rate`: readiness gained each beat
...
- Cost: 20
- Troop upgrades:
  - `Crippling Shots` (tier 3): on attack, reduce the target armor by 1 and rate by 1 for the battle
...
- Cost: 10
- Troop upgrades:
  - `R-selected` (tier 3): start of turn, gain +10 readiness per other Militia touching them; multiple Militia troops may enter the same Rift
  - `Dogpile` (tier 3): attacks against enemies engaged by at least 3 allies strike 1 extra time
  - `Crippling Hex` (tier 3): enemies who kill Militia gain 1 stack of Hex; enemies get -30% rate for each Hex stack
...
- Cost: 60
- Troop upgrades:
  - `On the Hunt` (tier 3): on attack, set the target readiness to 0; the first 2 kills against non-`Fading` enemies consume the corpse and summon a wolf there
  ...
  - `Hunter's Zeal` (tier 3): on kill, Rangers and allies adjacent to the killed enemy gain a stack of Zeal; allies gain 5 readiness per Zeal stack at end of turn
...
- `Forsaken` (tier 3): if this is the only troop on its side, gain +80% health, damage, and rate
- `Silvershot Doctrine` (tier 2): ranged and caster attacks gain +1 damage and +2 readiness per hex of distance; attacks made from max range make the target lose 30 readiness

### Goblins

- `Gallowsworn` (tier 1): on death, make 1 extra strike against a random touching enemy; on kill, all enemies lose 10 readiness
