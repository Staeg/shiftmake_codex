...
export const ABILITIES: Record<AbilityId, AbilityDefinition> = {
  ...
  'mycelial-beards': makeAbility({
    ...
    duration: instantDuration(),
    effects: [],
    shortText: 'Passive: incoming damage is split equally among Dwarven units on this hex after normal mitigation.',
  }),
  ...
  warcry: makeAbility({
    ...
    duration: instantDuration(),
    effects: [],
    shortText: 'Passive: when an enemy dies on this hex, all allies gain 1 damage for the battle.',
  }),
  berserk: makeAbility({
    id: 'berserk',
    label: 'Berserk',
  ...
  ensorcel: makeAbility({
    id: 'ensorcel',
    label: 'Ensorcel',
    trigger: { timing: 'passive' },
    duration: instantDuration(),
    effects: [],
    shortText: 'Passive: at battle start, mark an enemy that loses abilities and becomes the preferred Fae target.',
  }),
  glamour: makeAbility({
    id: 'glamour',
    label: 'Glamour',
...
export const FACTION_UPGRADES: Record<string, FactionUpgradeDefinition> = {
  ...
  'orc-warcry': {
    ...
    label: 'Warcry',
    tier: 3,
    description: 'Whenever an enemy unit dies on a hex with an Orc unit, all allied units gain 1 damage for the battle. Multiple Orcs can trigger this from the same death.',
    effects: [{ kind: 'addAbility', abilityId: 'warcry' }],
  },
  'orc-berserk': {
    id: 'orc-berserk',
    factionId: 'orc',
  ...
  'fae-ensorcel': {
    ...
    label: 'Ensorcel',
    tier: 1,
    description: 'At the start of combat, mark an enemy unit. It loses all abilities, and Fae units target it whenever it is in range. When it dies, another enemy is marked.',
    effects: [{ kind: 'addAbility', abilityId: 'ensorcel' }],
  },
  'fae-glamour': {
    id: 'fae-glamour',
    factionId: 'fae',
