...
export function getTroopUnlockCost(state, factionId, unitTypeId) {
    ...
    const startingUnitTypeId = factionId === 'human' || factionId === 'troll'
        ? 'soldier'
        : factionId === 'elf'
            ? 'archer'
            : 'militia';
    const unlockedNonStartingTroops = state.troops.filter((troop) => troop.factionId === factionId && troop.unlocked && troop.unitTypeId !== startingUnitTypeId).length;
