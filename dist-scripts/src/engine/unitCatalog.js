...
function summonEffect(unitTypeId, count, consumeFallenUnitCorpse = false, grantedAbilityIds = [], initialInitiative) {
...
export const ABILITIES = {
    ...
    'combined-arms-20': makeTripleStatAbility('combined-arms-20', 'Power of Friendship', 20, 'Start of battle: gain +20% health, +20% damage, and +20% speed for each other friendly troop type in this battle.', { repeatPerDistinctFriendlyTroopType: true }),
...
export const TROOP_TYPE_UPGRADES = {
...
const LEGACY_TROOP_TYPE_UPGRADE_IDS = {
...
export function getTroopTypeUpgrade(id) {
...
export function getSummonedUnitPreviews(ability, summonerFactionId) {
...
export const ALL_TROOP_UNLOCK_IDS = Object.keys(FACTIONS).flatMap((factionId) => UNLOCKABLE_UNIT_TYPE_IDS.map((unitTypeId) => getTroopUnlockId(factionId, unitTypeId)));
export const NATIVE_TROOP_UNIT_TYPE_IDS_BY_FACTION = {
...
export const NATIVE_TROOP_UNLOCK_IDS_BY_FACTION = Object.fromEntries(Object.keys(NATIVE_TROOP_UNIT_TYPE_IDS_BY_FACTION).map((factionId) => [
    factionId,
    NATIVE_TROOP_UNIT_TYPE_IDS_BY_FACTION[factionId].map((unitTypeId) => getTroopUnlockId(factionId, unitTypeId)),
]));
export const NATIVE_TROOP_UNLOCK_IDS = Object.values(NATIVE_TROOP_UNLOCK_IDS_BY_FACTION).flat().sort((left, right) => left.localeCompare(right));
...
export function getFactionNativeTroopUnlockIds(factionId) {
