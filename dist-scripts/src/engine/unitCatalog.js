...
export const ABILITIES = {
    ...
    'combined-arms-20': makeTripleStatAbility('combined-arms-20', 'Power of Friendship', 20, 'Start of battle: gain +20% health, +20% damage, and +20% speed for each other friendly troop class in this battle.', { repeatPerDistinctFriendlyTroopClass: true }),
    'forsaken-80': makeTripleStatAbility('forsaken-80', 'Forsaken 80', 80, 'Start of battle: if no other friendly troop classes are present, gain 80% health, damage, and speed.', { condition: 'forsaken' }),
...
export const RACE_UPGRADES = {
    ...
    'goblin-behavior': {
        ...
        tier: 1,
        description: 'On death: each goblin unit makes 1 extra strike against a random enemy on its hex. When a goblin gets a kill, all enemies on that hex lose 20 initiative.',
...
export const TROOP_CLASS_UPGRADES = {
    ...
    'militia-rat-behavior': {
        ...
        label: 'Rat Behavior',
        tier: 3,
        description: 'Start of turn: Militia gain +1 initiative for each other Militia on their hex. Militia do not count toward allied saturation limits.',
...
export function getRaceUpgrade(id) {
...
export function getTroopClassUpgrade(id) {
...
export function getTroopQuantityForCost(cost) {
    return fixedMax(fixed(TROOP_UNIT_BUDGET / Math.max(cost, 1)), 1);
}
export function composeBaseTroopDefinition(raceId, unitClassId) {
...
export function composeSummonedTroopDefinition(raceId, unitClassId) {
    const unitClass = getUnitClass(unitClassId);
    return {
        id: `${raceId}/${unitClassId}`,
        raceId,
...
export function getSummonedUnitPreviews(ability, summonerRaceId) {
...
export const TROOP_CATALOG = Object.values(RACES).reduce((acc, race) => {
...
export const ALL_TROOP_UNLOCK_IDS = Object.keys(RACES).flatMap((raceId) => UNLOCKABLE_UNIT_CLASS_IDS.map((unitClassId) => getTroopUnlockId(raceId, unitClassId)));
export const NATIVE_TROOP_UNIT_CLASS_IDS_BY_RACE = {
...
export const NATIVE_TROOP_UNLOCK_IDS_BY_RACE = Object.fromEntries(Object.keys(NATIVE_TROOP_UNIT_CLASS_IDS_BY_RACE).map((raceId) => [
    raceId,
    NATIVE_TROOP_UNIT_CLASS_IDS_BY_RACE[raceId].map((unitClassId) => getTroopUnlockId(raceId, unitClassId)),
]));
export const NATIVE_TROOP_UNLOCK_IDS = Object.values(NATIVE_TROOP_UNLOCK_IDS_BY_RACE).flat().sort((left, right) => left.localeCompare(right));
...
export function composeTroopDefinition(raceId, unitClassId) {
...
export function getBaseTroopCost(raceId, unitClassId) {
    return composeBaseTroopDefinition(raceId, unitClassId).cost;
}
export function getTroopUnlockId(raceId, unitClassId) {
    return `${raceId}/${unitClassId}`;
}
export function getRaceNativeTroopUnlockIds(raceId) {
    return [...NATIVE_TROOP_UNLOCK_IDS_BY_RACE[raceId]];
}
export function isNativeTroopUnlockId(troopUnlockId) {
