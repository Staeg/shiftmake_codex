...
function getEligiblePermutationUnitClassIds() {
  return filterEligiblePermutationUnitClassIds(Object.values(UNIT_CLASSES).map((unitClass) => unitClass.id));
}
function filterEligiblePermutationUnitClassIds(unitClassIds) {
  return Object.values(UNIT_CLASSES).filter((unitClass) => unitClassIds.includes(unitClass.id)).filter((unitClass) => !unitClass.attributes.includes("summoned")).map((unitClass) => unitClass.id).sort(compareTroopIds);
}
function resolvePermutationQuantity(unitClassId) {
  return Math.max(1, Math.round(120 / getUnitClass(unitClassId).cost));
}
function resolvePermutationTroops(unitClassIds = getEligiblePermutationUnitClassIds()) {
...
function generatePermutationTeams(teamSize, unitClassIds) {
...
function createEmptyPermutationAggregate(unitClassIds) {
...
function runPermutationBatch(teamSize, matchups, runCount, unitClassIds) {
  ...
  const results = matchups.map((matchup) => {
    const sweep = sweepBattleSeeds(
      (seed) => buildSimulationBattleInput(
        seed,
        matchup.left.troopIds.map(
          (troopId, index) => createUnitClassCombatant(troopId, {
            side: "player",
