...
function chooseRaceUpgradeIds(state, raceId, count, seed) {
  ...
  const available = Object.values(RACE_UPGRADES).filter((upgrade) => upgrade.raceId === raceId).map((upgrade) => upgrade.id).filter((upgradeId) => !state.raceUpgradeIds.includes(upgradeId));
  const selected = [];
...
function buildRaceUnlockOffer(state, cycleNumber, upgradeCount, troopUnlockChoiceCount) {
  ...
  const rng = createRng(deriveSeed(state.campaignSeed, cycleNumber * 30007 + upgradeCount * 101));
  ...
  const upgradeIdsByRaceId = Object.fromEntries(
    optionRaceIds.map((raceId) => [
      raceId,
      chooseRaceUpgradeIds(state, raceId, upgradeCount, deriveSeed(state.campaignSeed, cycleNumber * 31337 + raceId.length))
    ])
  );
  const troopUnlockIdsByRaceId = Object.fromEntries(
    optionRaceIds.map((raceId) => [
      raceId,
      chooseTroopUnlockIdsForRace(state, raceId, troopUnlockChoiceCount, deriveSeed(state.campaignSeed, cycleNumber * 37109 + raceId.length))
    ])
  );
  return {
    kind: "race_unlock",
...
function getRaceTroopClassUnlockOptions(state, raceId) {
  ...
  return [.../* @__PURE__ */ new Set([...native, ...defeated])].filter((troopUnlockId) => !ownedTroopUnlockIds.has(troopUnlockId));
}
function buildTroopClassUnlockOffer(state, cycleNumber, raceId, remainingChoices) {
  if (remainingChoices <= 0) {
    return null;
  }
  const optionTroopUnlockIds = getRaceTroopClassUnlockOptions(state, raceId).filter(
    (troopUnlockId) => rosterCanFitOpenRifts(state, troopUnlockId)
  );
...
function applyScheduledCycleUnlock(state) {
  ...
  const scheduled = state.cycleNumber === 3 ? { upgradeCount: 1, troopUnlockChoiceCount: 2 } : state.cycleNumber === 7 ? { upgradeCount: 2, troopUnlockChoiceCount: 3 } : null;
  if (!scheduled) {
    return state;
  }
...
function getOpeningRaceOptionIds(source) {
  if (source === void 0) {
    return Object.keys(RACES).slice(0, OPENING_RACE_OPTION_COUNT);
  }
  const seed = typeof source === "number" ? source : source.campaignSeed;
  return createRng(deriveSeed(seed, 45041)).shuffle(Object.keys(RACES)).slice(0, OPENING_RACE_OPTION_COUNT);
}
function getOpeningRaceStarterTroopUnlockIds(source) {
  const seed = source === void 0 ? 0 : typeof source === "number" ? source : source.campaignSeed;
  const rng = createRng(deriveSeed(seed, 46019));
  const raceIds = getOpeningRaceOptionIds(source);
  const optionsByRaceId = Object.fromEntries(
    raceIds.map((raceId) => [raceId, rng.shuffle(getRaceNativeTroopUnlockIds(raceId))])
  );
  ...
  if (!assignStarter(0, /* @__PURE__ */ new Set())) {
...
function getOpeningRaceStarterTroopUnlockId(source, raceId) {
...
function getAvailableUpgradeIds(state) {
  return [
    ...
    (upgradeId) => !state.raceUpgradeIds.includes(upgradeId) && !state.troopClassUpgradeIds.includes(upgradeId) && state.troops.some((troop) => upgradeAffectsTroop(upgradeId, troop))
  );
}
...
function buildTroopOffer(state) {
  const availableTroopUnlockIds = getAvailableTroopUnlockIds(state).filter(
    (troopUnlockId) => rosterCanFitOpenRifts(state, troopUnlockId)
  );
  ...
  const options = pickOfferOptions(
    deriveSeed(state.campaignSeed, state.cycleNumber * 10001 + state.troopOfferRolls + 1),
    [
...
function buildLeastUpgradedTroopUpgradeBucket(state, availableUpgradeIds, selected, rng) {
  ...
  for (const troop of shuffledTargetTroops) {
    const bucket = availableUpgradeIds.filter(
      (upgradeId) => !selected.has(upgradeId) && upgradeAffectsTroop(upgradeId, troop)
    );
    if (bucket.length > 0) {
      return bucket;
    }
...
function buildUpgradeOffer(state) {
  ...
  const rng = createRng(deriveSeed(state.campaignSeed, state.cycleNumber * 20003 + state.upgradeOfferRolls + 1));
  const selected = /* @__PURE__ */ new Set();
...
function getEssenceDraftCost(state) {
...
function startNewGame(seed = Date.now() >>> 0, gameMode = "campaign") {
  return buildInitialState(seed, gameMode);
}
function claimOpeningTroop(state, troopUnlockId) {
  ...
  if (state.troops.length >= 2 || !getOpeningRaceOptionIds(state).includes(raceId) || starterTroopUnlockId !== troopUnlockId || state.troops.some((troop) => troop.raceId === raceId || troop.unitClassId === unitClassId)) {
    return state;
  }
  return addTroopToRoster(state, troopUnlockId);
}
function unclaimOpeningTroop(state, troopUnlockId) {
  if (state.phase !== "opening_unlock" || !NATIVE_TROOP_UNLOCK_IDS.includes(troopUnlockId)) {
    return state;
  }
  const [raceId, unitClassId] = splitTroopUnlockId(troopUnlockId);
...
function startOpeningCampaign(state) {
...
function claimRaceUnlockOffer(state, raceId) {
...
function claimTroopClassUnlockOffer(state, troopUnlockId) {
  ...
  return nextOffer ? { ...nextState, activeTroopClassUnlockOffer: nextOffer } : { ...nextState, phase: "planning", activeTroopClassUnlockOffer: null };
}
function revealTroopOffer(state) {
...
function revealUpgradeOffer(state) {
  if (state.phase !== "planning" || state.essence < 1 || state.activeTroopOffer || state.activeUpgradeOffer) {
    return state;
  }
...
function revealEssenceDraft(state) {
...
function claimTroopOffer(state, troopUnlockId) {
  if (!state.activeTroopOffer || !state.activeTroopOffer.optionTroopUnlockIds.includes(troopUnlockId) || !getAvailableTroopUnlockIds(state).includes(troopUnlockId) || !rosterCanFitOpenRifts(state, troopUnlockId)) {
    return state;
  }
...
function claimUpgradeOffer(state, upgradeId) {
...
function validateAssignments(state) {
  ...
  const occupiedHumanTroopIds = new Set(
    state.gameMode === "contest" ? state.openRifts.filter((rift) => rift.controller === "human").flatMap((rift) => rift.occupyingTroopIds ?? []) : []
  );
  ...
  state.openRifts.filter((rift) => rift.state === "discovered").forEach((rift) => {
    const troops = getTroopsAssignedToRift(state, rift.id);
    const grouped = /* @__PURE__ */ new Map();
    const groupedUnitClasses = /* @__PURE__ */ new Map();
...
function getAssignmentIssue(state, troopId, riftId) {
  ...
  const heldRift = state.gameMode === "contest" ? state.openRifts.find((rift2) => rift2.controller === "human" && (rift2.occupyingTroopIds ?? []).includes(troop.id)) : null;
  if (heldRift) {
    return {
...
function assignTroopToRift(state, troopId, riftId) {
  ...
  return {
    ...state,
    troops: state.troops.map(
      (troop2) => troop2.id === troopId ? { ...troop2, assignmentRiftId: riftId } : troop2
    )
  };
}
function clearTroopAssignment(state, troopId) {
  if (state.phase !== "planning" || !state.troops.some((troop) => troop.id === troopId)) {
    return state;
  }
  if (state.gameMode === "contest" && state.openRifts.some((rift) => rift.controller === "human" && (rift.occupyingTroopIds ?? []).includes(troopId))) {
    return state;
  }
  return {
    ...state,
...
function randomlyAdvanceAiUnlocks(state) {
  ...
  const rng = createRng(deriveSeed(state.campaignSeed, state.cycleNumber * 44441 + 19));
...
function extractContestPlayerProgress(state) {
  return progressFromPseudoState(state);
}
function applyContestPlayerProgress(state, playerId, progress) {
  if (playerId === "human") {
    return {
      ...state,
      victoryPoints: progress.victoryPoints,
...
function getContestPlayerProgress(state, playerId) {
  return playerId === "human" ? extractContestPlayerProgress(state) : getContestAi(state);
}
function applyScheduledUnlockToContestProgress(state, progress) {
...
function getContestCombatantsForTroops(state, playerId, troops, side) {
  ...
  return troops.map(
    (troop) => resolveTroopCombatant(progress, troop, side, null, `${playerId}-${troop.id}`)
  );
}
...
function resolveContestBattle(state, rift, attackerId, attackingTroops, salt) {
  ...
  const battleInput = withBattleSideParticipants(buildBattleInputFromResolvedCombatants(
    ...
    enemy: defender.defenderId === "neutral" ? { kind: "neutral", label: "Neutral Guardians" } : {
...
function compareAiCandidateGroups(left, right) {
  return right.candidateScore - left.candidateScore || left.groupPower - right.groupPower || left.troops.length - right.troops.length || compareDeterministicTroopGroups(left.troops, right.troops);
}
function compareAiWinningGroups(left, right) {
  return right.winningGroupScore - left.winningGroupScore || left.groupPower - right.groupPower || left.troops.length - right.troops.length || compareDeterministicTroopGroups(left.troops, right.troops);
}
...
function buildAiWinningGroupsForRift(state, rift, readyTroops, options) {
  ...
  const battleCache = /* @__PURE__ */ new Map();
  const winningGroups = [];
  for (const candidate of buildAiCandidateGroupsForRift(state, rift, readyTroops, options).slice(0, options.maxBattleSimulationsPerRift)) {
    ...
    if (won === void 0) {
      won = resolveContestBattle(state, rift, "ai", candidate.troops, 91003).outcome === "victory";
...
function firstWinningAiAllocation(state, options) {
  ...
  let best = null;
  const selected = [];
  const usedTroopIds = /* @__PURE__ */ new Set();
  const scoreSelected = () => {
    ...
    const key = selected.map((choice) => `${choice.rift.id}:${choice.group.troops.map((troop) => troop.id).sort().join(",")}`).sort().join("|");
    return {
      choices: [...selected],
  ...
  const isBetterAllocation = (left, right) => {
    ...
    return left.score > right.score || left.score === right.score && (left.humanHeldCount > right.humanHeldCount || left.humanHeldCount === right.humanHeldCount && (left.victoryPoints > right.victoryPoints || left.victoryPoints === right.victoryPoints && (left.committedPower < right.committedPower || left.committedPower === right.committedPower && left.key.localeCompare(right.key) < 0)));
  };
  ...
  visit(0);
  return best ? new Map(best.choices.map((choice) => [choice.rift.id, choice.group.troops.map((troop) => troop.id)])) : empty;
}
function assignAiContestTroops(state, options) {
  ...
  const assignedTroopIds = /* @__PURE__ */ new Map();
...
function prepareContestAiForCycle(state, options = CONTEST_AI_WORKER_OPTIONS) {
  return getContestAi(prepareContestCycle(state, options));
}
function resolveContestAssignedRifts(state) {
  const records = [];
  state.openRifts.filter((rift) => rift.state === "discovered").forEach((rift, index) => {
    ...
    if (rift.controller === "neutral" || !rift.controller) {
      const humanGuardianRecord = humanTroops.length > 0 ? resolveContestBattle(state, rift, "human", humanTroops, 1e4 + index * 101) : null;
      const aiGuardianRecord = aiTroops.length > 0 ? resolveContestBattle(state, rift, "ai", aiTroops, 2e4 + index * 101) : null;
      ...
      if (humanGuardianRecord?.outcome === "victory" && aiGuardianRecord?.outcome === "victory") {
        records.push(resolveContestPvpBattle(state, rift, humanTroops, aiTroops, 3e4 + index * 101));
      }
      return;
    }
    if (rift.controller === "human" && aiTroops.length > 0) {
      records.push(resolveContestBattle(state, rift, "ai", aiTroops, 4e4 + index * 101));
      return;
    }
    if (rift.controller === "ai" && humanTroops.length > 0) {
      records.push(resolveContestBattle(state, rift, "human", humanTroops, 5e4 + index * 101));
    }
  });
...
function getGuardianUnlocksForRecord(state, record) {
  ...
  return guardianCombatants.map((combatant) => `${combatant.raceId}/${combatant.unitClassId}`).filter((troopUnlockId) => !isNativeTroopUnlockId(troopUnlockId)).filter((troopUnlockId) => !progress.unlockedTroopUnlockIds.includes(troopUnlockId)).filter((troopUnlockId) => !getOwnedTroopUnlockIds(progress).includes(troopUnlockId));
}
...
function isContestRecordVisibleToHuman(record) {
  ...
  return record.contest.kind === "guardian" || record.contest.kind === "pvp" || record.contest.attackerId === "human" || record.contest.defenderId === "human";
}
...
function applyContestCycleOutcomes(state, resolution) {
  ...
  const nextRifts = state.openRifts.map((rift) => {
    ...
    if (pvp) {
      ...
      const occupyingTroopIds = winnerId === "human" ? state.troops.filter((troop) => troop.assignmentRiftId === rift.id).map((troop) => troop.id) : winnerId === "ai" ? getContestAi(state).troops.filter((troop) => troop.assignmentRiftId === rift.id).map((troop) => troop.id) : [];
      return {
        ...rift,
  ...
  const occupiedByHuman = /* @__PURE__ */ new Map();
  const occupiedByAi = /* @__PURE__ */ new Map();
  ...
  nextState.replayIndex.forEach((entry) => {
    if (kept.length >= 40 || totalBytes + entry.estimatedBytes > 4e6) {
...
function resolveAssignedRifts(state, preparedContestAi) {
  if (state.gameMode === "contest") {
  ...
  const records = state.openRifts.filter((rift) => rift.state === "discovered").flatMap((rift) => {
    const troops = getTroopsAssignedToRift(state, rift.id);
    if (troops.length === 0) {
      return [];
    }
...
function applyCycleOutcomes(state, resolution) {
  if (state.gameMode === "contest") {
    return applyContestCycleOutcomes(resolution.preparedState ?? state, resolution);
  }
  const newlyUnlockedTroopUnlockIds = [
    ...new Set(
      resolution.records.filter((record) => record.outcome === "victory").flatMap((record) => record.battleInput.enemyCombatants.map((combatant) => `${combatant.raceId}/${combatant.unitClassId}`)).filter((troopUnlockId) => !isNativeTroopUnlockId(troopUnlockId)).filter((troopUnlockId) => !state.unlockedTroopUnlockIds.includes(troopUnlockId)).filter((troopUnlockId) => !getOwnedTroopUnlockIds(state).includes(troopUnlockId))
    )
  ];
...
export {
  ...
  CONTEST_AI_SYNC_FALLBACK_OPTIONS,
  CONTEST_AI_WORKER_OPTIONS,
  CONTEST_FINAL_CYCLE,
  ...
  getOpeningRaceStarterTroopUnlockId,
  getOpeningRaceStarterTroopUnlockIds,
  prepareContestAiForCycle,
