...
import { canUpgradeStat, createTroopInstance, DEFEAT_RECOVERY, getFactionTroops, getFactionUnlockCost, getTroopAddUnitCost, getTroopById, getTroopEffectiveDefinition, getTroopStatUpgradeCost, getTroopUnlockCost, getTroopsAssignedToRift, isFactionUnited, resolveTroopCombatant, tickRecovery, VICTORY_RECOVERY, } from './army';
...
function allFactionIds() {
    return Object.keys(FACTIONS);
}
export function getStartingFactionUnitType(factionId) {
    return {
        human: 'soldier',
        elf: 'archer',
        goblin: 'militia',
...
function getRewardUpgradePool(state) {
...
function buildInitialState(seed, options) {
...
function nextTroopIndex(state, factionId, unitTypeId) {
    return state.troops.filter((troop) => troop.factionId === factionId && troop.unitTypeId === unitTypeId).length + 1;
}
function buildReplayIndexEntry(cycleNumber, replay) {
...
function buildRewardChoicesForRift(state, riftId, upgradeChoiceBatchCount, blueprintChoiceCountByTier) {
...
export function startNewGame(seed = Date.now() >>> 0, options) {
    return buildInitialState(seed, options);
}
export function chooseStartingFaction(state, factionId) {
...
export function assignTroopToRift(state, troopId, riftId) {
    return {
        ...state,
        troops: state.troops.map((troop) => troop.id === troopId ? { ...troop, assignmentRiftId: troop.assignmentRiftId === riftId ? null : riftId } : troop),
    };
}
...
export function unlockFaction(state, factionId) {
...
export function unlockTroopType(state, factionId, unitTypeId) {
...
export function buyTroopUnit(state, troopId) {
...
export function buyTroopStatUpgrade(state, troopId, stat) {
    const troop = getTroopById(state, troopId);
    ...
    return {
        ...state,
        resources: { ...state.resources, gold: fixed(state.resources.gold - cost) },
        troops: state.troops.map((entry) => entry.id === troopId
            ? {
                ...entry,
                statUpgradeLevels: {
...
export function buyFactionUpgrade(state, upgradeId) {
...
export function resolveAssignedRifts(state) {
    const records = state.openRifts
        ...
        const battleInput = buildBattleInputFromResolvedCombatants(battleSeed, rift.id, rift.tier, rift.mutatorIds, rift.saturation, troops.map((troop) => resolveTroopCombatant(state, troop, 'player')), rift.enemyArmy);
...
export function applyCycleOutcomes(state, resolution) {
    let nextState = {
        ...
        troops: tickRecovery(state.troops.map((troop) => {
        ...
        openRifts: markExistingRiftsInactive(state.openRifts.map((rift) => {
    ...
    resolution.records.forEach((record) => {
        ...
        if (record.outcome === 'victory') {
            ...
            nextState.pendingRewardChoices = [
                ...nextState.pendingRewardChoices,
                ...buildRewardChoicesForRift(nextState, record.riftId, record.rewardPackage.upgradeChoiceBatches, record.rewardPackage.blueprintChoiceCountByTier),
            ];
        }
    });
...
export function claimRewardChoice(state, rewardChoiceId, optionId) {
...
export function advanceFromRewards(state) {
