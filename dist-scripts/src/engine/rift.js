...
function getCycleTierSchedule(cycleNumber) {
...
function generateCycleRifts(state) {
...
function getContestCycleTierSchedule(cycleNumber) {
  if (cycleNumber === 1) {
    return [1, 1, 1];
  }
  if (cycleNumber === 3) {
...
function generateContestCycleRifts(state) {
  const tiers = getContestCycleTierSchedule(state.cycleNumber);
  const cycleSeed = deriveSeed(state.campaignSeed, state.cycleNumber * 9973 + 17);
  const cycleMutatorAssignments = buildCycleMutatorAssignments(tiers, cycleSeed);
  return tiers.map((tier, index) => {
    const riftSeed = deriveSeed(cycleSeed, index + 1);
    return {
...
function getMutatorLabels(mutatorIds) {
  return mutatorIds.map((id) => getMutator(id).label);
}
export {
  deriveSeed,
  generateContestCycleRifts,
  generateCycleRifts,
  getContestCycleTierSchedule,
  getCycleTierSchedule,
  getMutatorLabels
};
