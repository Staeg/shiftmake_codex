...
function getUnownedUpgradeIds(state) {
...
function getClaimableTroopUnlockIds(state) {
  ...
  return [.../* @__PURE__ */ new Set([...nativeTroopUnlockIds, ...latentTroopUnlockIds])];
}
function getOwnedTroopUnlockIds(state) {
  return state.troops.map((troop) => `${troop.raceId}/${troop.unitClassId}`);
}
function getAvailableTroopUnlockIds(state) {
...
function describeTroopUnlock(troopUnlockId) {
...
function troopClassUpgradeAffectsTroop(upgradeId, troop) {
  ...
  return upgrade.effects.some((effect) => {
    ...
    if (effect.kind === "replaceAbility") {
      return definition.abilities.some((ability) => ability.id === effect.removeAbilityId) && !definition.abilities.some((ability) => ability.id === effect.addAbilityId);
    }
    if (effect.kind === "addAttribute") {
      return !definition.attributes.includes(effect.attribute);
    }
...
function upgradeAffectsTroop(upgradeId, troop) {
...
export {
  describeTroopUnlock,
  getAllUpgradeIds,
  getAvailableTroopUnlockIds,
  getClaimableTroopUnlockIds,
  getOwnedTroopUnlockIds,
  getUnownedUpgradeIds,
  upgradeAffectsTroop
};
