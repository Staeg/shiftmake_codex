...
function getTroopById(state, troopId) {
...
function getTroopsAssignedToRift(state, riftId) {
  return state.troops.filter((troop) => troop.assignmentRiftId === riftId);
}
function isRaceUnited(state, raceId) {
  return state.raceUpgradeIds.map(getRaceUpgrade).filter((upgrade) => upgrade.raceId === raceId).some((upgrade) => upgrade.effects.some((effect) => effect.kind === "addAbility" && getAbility(effect.abilityId).overworldEffectId === "united"));
}
...
function applyRaceUpgradeEffects(state, raceId, role, stats, abilities, attributes) {
  let nextStats = { ...stats };
  const nextAbilities = [...abilities];
  const nextAttributes = [...attributes];
  state.raceUpgradeIds.map(getRaceUpgrade).filter((upgrade) => upgrade.raceId === raceId).forEach((upgrade) => {
...
function applyRaceUpgradeEffectsDetailed(state, raceId, role, stats, abilities, attributes) {
  let nextStats = { ...stats };
  const nextAbilities = [...abilities];
  const nextAttributes = [...attributes];
...
function applyTroopClassUpgradeEffects(state, unitClassId, stats, abilities, attributes) {
  ...
  const nextAttributes = [...attributes];
  state.troopClassUpgradeIds.map(getTroopClassUpgrade).filter((upgrade) => upgrade.unitClassId === unitClassId).forEach((upgrade) => {
    upgrade.effects.forEach((effect) => {
      if (effect.kind === "addAbility") {
        const ability = getAbility(effect.abilityId);
...
function getResolvedStatBreakdowns(state, troop, side, enemyTier = null) {
  const unitClass = getUnitClass(troop.unitClassId);
...
function getTroopQuantityBreakdown(troop) {
  const race = getRace(troop.raceId);
  const unitClass = getUnitClass(troop.unitClassId);
  ...
  if (costMultiplier !== 1 || costFlat !== 0) {
    const costText = costMultiplier !== 1 ? `Cost x${formatFixed(costMultiplier)}` : `Cost ${costFlat > 0 ? "+" : ""}${formatFixed(costFlat)}`;
    lines.push({
...
function getEnemyStatBreakdowns(raceId, unitClassId, tier) {
...
function resolveTroopCombatant(state, troop, side, enemyTier = null, combatantId = troop.id) {
  const base = composeBaseTroopDefinition(troop.raceId, troop.unitClassId);
...
function resolveEnemyCombatant(raceUpgradeIds, troopClassUpgradeIds, raceId, unitClassId, tier, combatantId) {
...
function getTroopEffectiveDefinition(state, troopId) {
  return resolveTroopCombatant(state, getTroopById(state, troopId), "player");
}
function getTroopStatusCounts(state) {
  let active = 0;
  let recovering = 0;
  let idle = 0;
...
export {
  ...
  VICTORY_RECOVERY,
  createTroopInstance,
  getEnemyStatBreakdowns,
  getRaceTroops,
  getResolvedStatBreakdowns,
  getRiftSummaryTroops,
  getTroopById,
